function varargout=import_stl_fast_original(filename,mode)
...
%ThirdLine=textscan(fid, , 'HeaderLines', 2);
for i=1:4
tline = fgetl(fid);
end
fclose(fid);
...
fclose(fid);
if isempty(C{4})
fid=fopen(filename, 'r'); % Reopen the file, assumes STL ASCII format.
fmt = '%*s %*s %f32 %f32 %f32 \n %*s %*s \n %*s %f32 %f32 %f32 \n %*s %f32 %f32 %f32 \n %*s %f32 %f32 %f32 \n %*s \n %*s \n';
C=textscan(fid, fmt, 'HeaderLines', 1);
fmt_mode=2;
end
